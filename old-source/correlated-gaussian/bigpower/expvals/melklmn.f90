SUBROUTINE      melklmn(n,vechLk,mk,vechLl,ml,Sym,Skl,Tkl,Vkl, &
                        R1kl,R1skl,dSk,dSl,dTk,dTl,dVk,dVl)
  ...
  !
  !***Last Modified Date***
  ! Thu Mar 25 17:45:58 MST 1999
  ! added R1kl and R1skl
  ! the integrals <k|r|l> and <k|r^2|l>
  ! inserted code after the overlap
  ! 
  !***************************
  !=======
  !Purpose
...
!!$  Skl = Skl * gm1(mk,ml)
  ...
  mko2 = mk/2
  mlo2 = ml/2
  Skl = ( invAkl(1,1)/invAk(1,1) )**(mko2) * gm1(mko2,mlo2)
  Skl = Skl * ( ( invAkl(1,1)/invAl(1,1) )**(mlo2) * gm1(mko2,mlo2) )
  Skl = Skl * DSQRT( DBLE(2**(3*n))*(DABS( detLk*detLl/detAkl )**3) )
  ...
  !********************************************************
  ! compute R1kl and R1skl (expectation values for bond length)
  !******************************************************** 
  R1kl = Skl*0.5d0*DBLE(mk+ml+2)*gm2((mk+ml)/2)*DSQRT(invAkl(1,1))
  R1skl = Skl*0.5d0*DBLE(mk+ml+3)*invAkl(1,1)
  !
  !********************************************************
