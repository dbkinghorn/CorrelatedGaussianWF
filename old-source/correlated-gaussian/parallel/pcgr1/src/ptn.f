...
c
c     Started work on parallelizing this code
c     Thu Nov 18 16:08:21 MST 1999
c     Added rank to arg list
c     All IO now goes to rank 0 process
C
C******************************************************************
      SUBROUTINE TN (IERROR, N, X, F, G, W, LW, SFUN, rank)
      IMPLICIT          DOUBLE PRECISION (A-H,O-Z)
      INTEGER           IERROR, N, LW, rank
...
C PRINT THE RESULTS
C
c only print from rank 0 process
      if(rank .EQ. 0)then
...
C
     ...
     *            MSGLVL, MAXIT, MAXFUN, ETA, STEPMX, ACCRCY, XTOL,rank)
      IMPLICIT          DOUBLE PRECISION (A-H,O-Z)
      INTEGER           MSGLVL, N, MAXFUN, IFAIL, LW, rank
...
C
     ...
     *     ACCRCY,GTPNEW,GNORM,XNORM,rank)
20    CONTINUE
...
10    CONTINUE
      ...
      GTG = DDOT(N,G,1,G,1)
      IF (MSGLVL .GE. 1)
     *    CALL MONIT(N,X,FNEW,G,NITER,NFTOTL,NFEVAL,LRESET,IPIVOT,rank)
C
...
150   F = OLDF
     ...
     *     F,G,NITER,NFTOTL,NFEVAL,IRESET,IPIVOT,rank)
C
C SET IFAIL
C
160   IFAIL = NWHY
...
C
C
      SUBROUTINE MONIT(N,X,F,G,NITER,NFTOTL,NFEVAL,IRESET,IPIVOT,rank)
C
C PRINT RESULTS OF CURRENT ITERATION
C
      IMPLICIT         DOUBLE PRECISION (A-H,O-Z)
      DOUBLE PRECISION X(N),F,G(N),GTG
      INTEGER          IPIVOT(N),rank
C
      GTG = 0.D0
      DO 10 I = 1,N
...
C
     ...
     *     GTP,GNORM,XNORM,rank)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER MODET,N,NITER,IPIVOT(1),rank
...
20       CONTINUE
         ...
         CALL NDIA3(N,EMAT,V,GV,R,VGV,MODET,rank)
C
...
C
C
      SUBROUTINE NDIA3(N,E,V,GV,R,VGV,MODET,rank)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C
     ...
     *     W(LSK),W(LYK),W(LSR),W(LYR),MODET,UPD1,rank)
      RETURN
      END
      SUBROUTINE INITP3(DIAGB,EMAT,N,LRESET,YKSK,YRSR,BSK,
     *     SK,YK,SR,YR,MODET,UPD1,rank)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
CLSOUT
     ...
     *     XW,FW,GW,U,A,B,TOL,RELTOL,SCXBND,XBND,rank)
C
