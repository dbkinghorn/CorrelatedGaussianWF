...
%TCIDATA{LastRevised=Thu Jan 02 11:32:55 1997}
...
_k$, if $\phi _k$ is to be square integratable, i.e. admit a norm, then $A_k$
must be positive definite. The simpilest way to insure this condition is to
write $A_k$ in Cholesky factored form, 
\begin{equation}
...
N-1\right) /2>3N-3.$ These relations are shown explicitly in table [\ref
{Ntab}]. These differences in the number of coordinates do not present any
real problems since transformations between variables $\mathbf{r}$, $\left\{
...
using matrix differential calculus[ref]. Introducing an over-bar notation
for the Kronecker product with the $3\times 3$ identity, i.e. $A_k\otimes
I_3=\bar{A}_k\,\,$, and defining $G=\left( \sum_{i\leq j}m_{kij}\left( 
\mathbf{r}^{\prime }\bar{J}_{ij}\mathbf{r}\right) ^{-1}\bar{J}_{ij}\right) $%
), the gradient with respect to $\mathbf{r}$ is then, 
\begin{equation}
...
\end{eqnarray}
In many cases it will be more convenient to consider derivatives with
respect to the distance coordinates $r_{ij}.$ The individual derivatives are
then, 
\begin{equation}
\frac{\partial \phi _k}{\partial r_{ij}}=\left(
m_{kij}r_{ij}^{-1}-2B_{kij}r_{ij}\right) \phi _k,
\end{equation}
\begin{equation}
\frac{\partial ^2\phi _k}{\partial r_{ij}^2}=\left(
m_{kij}r_{ij}^{-1}-2B_{kij}r_{ij}\right) ^2\phi _k-\left(
m_{kij}r_{ij}^{-2}+2B_{kij}\right) \phi _k,
\end{equation}
where $B_{kij}$ is the $ij$-th component of the matrix $B_k$ in eqn(\ref
{Btran}).
