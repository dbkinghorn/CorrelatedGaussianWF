...
c     ..
c     .. Local Scalars ..
      INTEGER             I,J,K,CI,CJ,C,KI,KJ,FLAG,CNT
C      DOUBLE PRECISION    SEV
c     ..
c     .. Local Arrays ..
...
c     This is a label for a goto from the lin dep check after the eng calc
1111  CONTINUE
c
...
c     Do it the easy way and just compute all of them
      CALL DSYGV( 1, 'V', 'L', NB, H, NBMX, S, NBMX, EVAL,
     &             WORK, LWORK, INFO ) 
...
c
C
C	THE SCALE FACTOR IS ONE BY NORM OF EVEC
c     make scale factor EVEC'*S*EVEC (S IS LL' NOW)
      CALL DCOPY( NB, EVEC(1,1), 1, V1, 1)   
      CALL DTRMV( 'L', 'T', 'N', NB, S, NBMX, V1, 1 )
      SCALE = DDOT( NB, V1, 1, V1, 1 )
      SCALE = ONE/SCALE
     
c
c     This is a check and fix for lin dep
      FLAG = 0
      DO I=1,NB
...
c              FLAG = 1
c              DO J=1,1
c                 X((I-1)*NN+J) = X((I-1)*NN+J)*DRANDM(I*J)+DRANDM(I) 
c              END DO
c              RETURN
          END IF
      END DO
