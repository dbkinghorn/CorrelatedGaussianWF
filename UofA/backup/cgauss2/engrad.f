...
c     compute cholesky factorization of S      
      CALL DPOTRF( 'L', NB, S, NBMX, INFO )
c     
c     transform to standard form Hx=ySx --> Cz=yz
c     C is in H
      CALL DSYGST( 1, 'L', NB, H, NBMX, S, NBMX, INFO )
c
c     tridiagonalize C
      CALL DSYTRD( 'L', NB, H, NBMX, D, E, TAU, WORK, LWORK, INFO )
c
c     now compute the eigenvalue # WEV
      CALL DSTEBZ('I', 'E', NB, VL, VU, WEV, WEV, ABSTOL, D, E, M,
...
c     This gives the eigenvector for the tridiagonal system                                
      CALL DSTEIN( NB, D, E, M, EVAL, IBLOCK, ISPLIT, EVEC,
     &            NBMX, WORK, IWORK, IFAIL, INFO )
c     convert to the factored system      
      CALL DORMTR( 'L', 'L', 'N', NB, M, H, NBMX, TAU, EVEC, NBMX,
     &            WORK, LWORK, INFO )     
c     convert to original system
      CALL DTRTRS('L', 'T', 'N', NB, M, S, NBMX, EVEC, NBMX, INFO )
...
C      CALL DCOPY( NB, EVEC(1,1), 1, V1, 1)   
C      CALL DTRMV( 'L', 'T', 'N', NB, S, NBMX, V1, 1 )
C      SCALE = DDOT( NB, V1, 1, V1, 1 )
      SCALE = ONE



C     THIS IS A FIX FOR LIN DEP
      DO I=1,NB
          IF( ABS(EVEC(I,1)) .GT. 100.0D0 ) THEN
              RETURN
C              X(I*NN) = X(I*NN) + I*(ONE/NB)
C              GOTO 1111
          END IF
      END DO
