      SUBROUTINE EGFIXEV2( NX, X, ENG, GRAD)
c
c --- Donald B. Kinghorn
c     Dept of Chemistry
...
c ABSTOL  convergence tolarence for the eigenvalue
     ...
     &                    T(NBMX,NBMX), V(NBMX,NBMX)
      COMMON /DATA/ N, NB, NN, NSYMM, MASS, CHRG, SYMMC,SYMM
      COMMON /EIG/ WEV, ABSTOL, EVEC, SCALE, T, V
*********************************************************************
*********************************************************************
*********************************************************************        
...
C
C     SET EVEC TO SOME VALUES
      DO I=1,NB
          EVEC(I,1) = .05D0 + ONE/(DBLE(I))
      END DO
...
C     USING V1 AND V2 FOR STORAGE
      ...
      DO I=1,NB
          ...
          V2(I) = ZERO
          DO J=1,NB
              V1(I) = V1(I) + EVEC(J,1) * H(I,J)
              V2(I) = V2(I) + EVEC(J,1) * S(I,J)
          END DO
      END DO
      ...
      DO I=1,NB
          SUMH = SUMH + V1(I) * EVEC(I,1)
          SUMS = SUMS + V2(I) * EVEC(I,1)
      END DO 
