...
#################################################
#
# This script takes the add site form data from
# boss.html and sets up a course web site
#
#################################################
...
$semesterCode = $query->param('semesterCode'); # Dept. semester code [972]
$courseName = $query->param('courseName');     # Name of the course
$section = $query->param('section');           # Section number
$insName = $query->param('insName');           # Instructor name
$insEmail = $query->param('insEmail');         # Instructor E-mail
$menuID = $query->param('menuID');             # Menu level ID
$option = $query->param('optMenu');            # Options add overwrite or delete
...
{    
    print $query->header();
    print $query->start_html(-title=>'Bad Data',
                             -BGCOLOR=>"#D3D3D3");
    print "\n<H1>Bad data</H1>\n";
...
## Make (or delete) the course directories

if( $option =~ m/-d/ )          # Delete the site if it exists
{
    $logEntry .= 
...
else
{
    ...
    $logEntry .= 
        `cw_setlabel.pl -t '\<!--courseDir--\>' -n "$coursePath/" -p "$coursePath/instructor" @instructorFiles`;
    $logEntry .= 
        `cw_setmenu.pl $option -b "$menuBegin" -e "$menuEnd" -t "$menuEntry" "$courseList"`;
    $logEntry .= 
        `cw_setmenu.pl $option -b "$menuBegin" -e "$menuEnd" -t "$ImenuEntry" "$IcourseList"`;
    writeLog( $logEntry, $bossLog );
    sendHtml( "Site created" );
    if( checkEmail( $insEmail ) ){ sendEmail( $insEmail, "CourseWeb B.O.S.S.", $insSubject, $insMsg );  }
    }
}
