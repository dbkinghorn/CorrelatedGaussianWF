PROGRAM potfit2p
  ...
  !
  !***Last Modified Date***
  ! Mon Oct 19 16:56:20 MST 1998
  ! got rid of IMSL dependency (yuk)
  !
  ! 27 Aug. 1996
  !***************************
  !====================================================================
  ...
  !
  ! potfit2p reads an input file containing the number and 
  ! types (powers m) of 
  ! of expansion functions to be used, and an initial guess for the 
  ! nonlinear (ak) parameters.
  !
  ! The numerical potential is accesed via the module fitdata 
  ! which contains
  ! r the set of points where the potential was evaluated, y, 
  ! the values at r,
  ...
  INTEGER                               :: npow,nb,i,j,cnt,INFO,LOPTW      
  INTEGER, ALLOCATABLE, DIMENSION(:,:)  :: powers
  DOUBLE PRECISION, DIMENSION(:), ALLOCATABLE :: b,b0,XSCALE,OPTW
  ...
  !       (2/pi)^3/2  2^(2m)/(2m+1)!!
  !
  nrm = DSQRT( 0.50794908747392775829D0 * &
        (2.0D0**(2 * mvec))/facfac(mvec) )
  ! Look Ma no loops !!
  !nrm = 1
  !PRINT *, nrm
  !PRINT *, mvec
  !
  ! Load the numerical potential data (nr and dat)
  !
  ! open file numpot.dat thsi file must exist in the calling directory
  OPEN(UNIT=10, FILE="numpot.dat", STATUS="OLD", ACTION="READ")

  ! read the first line of numpot.dat to get 
  ! the number of fit points nr
  READ(10,*) nr
  ...
  b0 = b
  !call pcgfit(nb,b,chisqr)
  !CALL uncmnd(nb,b0,pcgfit,b,chisqr,INFO,OPTW,LOPTW)
