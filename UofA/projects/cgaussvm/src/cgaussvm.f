...
c     OLDENG, ALPHA, OLDX(NXMX)
      DOUBLE PRECISION    ETA, ACCRCY, XTOL, STEPMX, DSQRT,
     &                    VIRIAL, SUMT, SUMV  
      DOUBLE PRECISION	RN
c     ..
c     .. Local Arrays .. 
      DOUBLE PRECISION    X(NXMX), XAC(NXACMX), GRAD(NXACMX)
c     , W(NLW)

c     Variables for A21VM()
      integer           IFN,IG,IPR
      double precision  BH(NXACMX,NXACMX), W1(NXACMX),W2(NXACMX),
     &                  W3(NXACMX), W4(NXACMX)
...
c      CALL LMQN (IERROR, NXAC, XAC, ENG, GRAD, W, LW, ENGRADRC, 
c     &     MSGLVL, MAXIT, MAXFUN, ETA, STEPMX, ACCRCY, XTOL)

c     How about a simple variable metric optimization

c     max function calls
      IFN = 1000
c     output unit number
      IPR = 6
      write(*,*) "before call"
      call A21VM(NXAC, XAC, BH, NXACMX, W1, W2,
     &           W3, W4, IFN, IG,
     &           IPR, ENG, ENGRADRC )
