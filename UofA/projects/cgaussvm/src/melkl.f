      SUBROUTINE  MELKL( N, NMAX, VLK, VLL, MASS, CHRG,P,
...
c     ..Array Arguments..
      DOUBLE PRECISION    VLK(*), VLL(*), CHRG(*), MASS(NMAX,*),
     &                    P(NMAX,*),
     &                    DSK(*), DSL(*), DTK(*), DTL(*),
     &                    DVK(*), DVL(*),    
     &                    LK(NMAX,*), LL(NMAX,*),
     &                    AK(NMAX,*), AL(NMAX,*),
     &                    INVAKL(NMAX,*), MAK(NMAX,*), MAL(NMAX,*),
     &                    IAAK(NMAX,*), IAAL(NMAX,*),
     &                    IALK(NMAX,*), IALL(NMAX,*),
     &                    TMP1(NMAX,*), TMP2(NMAX,*),
     &                    TMPV1(*), TMPV2(*),
...
C     L,C,R, NOT USED FOR NEW DV CODE
C      INTEGER             L, C, R
      DOUBLE PRECISION    DETLK, DETLL, DETAKL, SUM, SUM0
...
c
c      CALL DPOTRI( 'L', N, INVAKL, NMAX, INFO )
c
...
c      DO I=1,N-1
c          DO J=I+1,N
c              INVAKL(I,J) = INVAKL(J,I)
c          END DO
c      END DO

C     do the inverse without the LAPACK call
c     [invert the lower triangle matrix]
c     copy INVAKL to TMP1 for now ...yea I know this is bad
      do j=1,n
         do i=1,n
            TMP1(i,j)=INVAKL(i,j)
         end do
      ...
      DO i=1,n
         TMP1(i,i) = ONE/TMP1(i,i)
      DO j=i+1,n
         SUM0 = ZERO
         DO k=i,j-1
           SUM0 = SUM0 - TMP1(j,k) * TMP1(k,i)
         END DO
        TMP1(j,i) = SUM0 / TMP1(j,j)
        TMP1(i,j) = ZERO ! zero out the upper triangle for convenience
      END DO
      END DO

c     we now have lower triangleINVAKL
c     now do invAkl = TMP1'*TMP1
      do i=1,n
         do j=1,n
            INVAKL(i,j) = ZERO
         end do
      ...
      DO j=1,n
         DO k=1,n
            DO i=1,n
               INVAKL(i,j) = INVAKL(i,j) + TMP1(k,i) * TMP1(k,j)
            END DO
         END DO
      END DO
